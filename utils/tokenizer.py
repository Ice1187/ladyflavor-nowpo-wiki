...
def tokenize_sentences(sentence_list, ws):
    ...
    stripped_word_sentence_list = [[word.strip() for word in words if word.strip()] for words in word_sentence_list]
...
def _tokenized_all():
  if len(sys.argv) != 2:
    print(f'{argv[0]} ./ckiptagger/data')
    return

  ws = get_ws_model(sys.argv[1])
  
  for transcript_file in Path('../tw_transcripts/').glob('EP*.txt'):
    print(transcript_file)
    with open(transcript_file, 'r') as f:
      texts = [text.strip() for text in f.read().strip().split('\n')]

    output_file = str(transcript_file).replace('tw_transcripts', 'tokenized_tw_transcripts')
